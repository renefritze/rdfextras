...
class SPARQLXMLWriter:
    ...
    def __init__(self, output, encoding='utf-8'):
        ...
        writer.startElementNS(
            (SPARQL_XML_NAMESPACE, 'sparql'),
             'sparql', AttributesNSImpl({}, {}))
        self.writer = writer
        self._output = output
    ...
    def write_header(self, allvarsL):
        self.writer.startElementNS(
            (SPARQL_XML_NAMESPACE, 'head'),
             'head', AttributesNSImpl({}, {}))
        for i in range(0, len(allvarsL)):
            ...
            self.writer.startElementNS(
                (SPARQL_XML_NAMESPACE, 'variable'),
                 'variable', AttributesNSImpl(attr_vals, attr_qnames))
    ...
    def write_results_header(self):
        self.writer.startElementNS(
            (SPARQL_XML_NAMESPACE, 'results'),
             'results', AttributesNSImpl({}, {}))
        self._results = True

    def write_start_result(self):
        self.writer.startElementNS(
            (SPARQL_XML_NAMESPACE, 'result'),
             'result', AttributesNSImpl({}, {}))
    ...
    def write_binding(self, name, val):
        ...
        self.writer.startElementNS(
            (SPARQL_XML_NAMESPACE, 'binding'),
             'binding', AttributesNSImpl(attr_vals, attr_qnames))

        if isinstance(val, URIRef):
            self.writer.startElementNS(
                (SPARQL_XML_NAMESPACE, 'uri'),
                'uri', AttributesNSImpl({}, {}))
            ...
            self.writer.startElementNS(
                (SPARQL_XML_NAMESPACE, 'literal'),
                 'literal', AttributesNSImpl(attr_vals, attr_qnames))
            self.writer.characters(val)
            self.writer.endElementNS(
